...
func TestPin_UnpinnedToPinned_CallsToggle(t *testing.T) {
	...
	require.NoError(t, runPin(context.Background(), &PinOptions{}, &cmdutil.FormatOptions{Mode: cmdutil.FormatHuman}, svc, "kb_abc", true))
...
func TestUnpin_PinnedToUnpinned_CallsToggle(t *testing.T) {
	...
	svc := &fakePinSvc{current: sdk.KnowledgeBase{IsPinned: true}}
	require.NoError(t, runPin(context.Background(), &PinOptions{}, &cmdutil.FormatOptions{Mode: cmdutil.FormatHuman}, svc, "kb_abc", false))
...
func TestPin_NotFound(t *testing.T) {
	...
	err := runPin(context.Background(), &PinOptions{}, &cmdutil.FormatOptions{Mode: cmdutil.FormatHuman}, svc, "kb_missing", true)
	require.Error(t, err)
...
func TestPin_ToggleError(t *testing.T) {
	...
	err := runPin(context.Background(), &PinOptions{}, &cmdutil.FormatOptions{Mode: cmdutil.FormatHuman}, svc, "kb_abc", true)
	require.Error(t, err)
	var typed *cmdutil.Error
	require.ErrorAs(t, err, &typed)
